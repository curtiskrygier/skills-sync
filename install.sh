...
# ── Claude Code + Gemini CLI global config ───────────────────────────────────
...
When syncing skills (running \`skills-sync\`), first use the \`d2-architect-standard\` skill \
to generate or update a \`diagram.d2\` and \`diagram.svg\` for each skill whose SKILL.md \
has changed. Render with: d2 -t 0 <skill>/diagram.d2 <skill>/diagram.svg\
""".format(skills_dir=SKILLS_DIR)

def upsert_section(path_str, tool_name):
    p = pathlib.Path(path_str)
    content = p.read_text() if p.exists() else ""
    pattern = r'## Skills Library[\s\S]*?(?=\n## |\Z)'
    if re.search(pattern, content):
        content = re.sub(pattern, section, content)
        print(f"{tool_name}: updated Skills Library section in {path_str}")
    else:
        content = content.rstrip('\n') + ('\n\n' if content else '') + section + '\n'
        print(f"{tool_name}: added Skills Library section to {path_str}")
    p.parent.mkdir(parents=True, exist_ok=True)
    p.write_text(content)

upsert_section("$CLAUDE_MD", "Claude Code")
upsert_section("$GEMINI_MD", "Gemini CLI")
PYEOF
